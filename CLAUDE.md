...
## Lead webhook fixed (2026-09-23)

Claudia lead webhook FIXED 2026-09-23 02:40 UTC (Sidd's "yes do it"). Cause: hc-dashboard commit 27e22bf (2026-08-27) made /webhooks/ms-graph require clientState equal to the worker secret MS_GRAPH_CLIENT_STATE; first deployed about 2026-09-06; the secret was never set (503 'Webhook unavailable') and the poller never sent a clientState. Last good forward 2026-09-06 18:28 UTC, first 503 2026-09-07 00:29 UTC, about 2,400 refusals a day; newest dashboard website lead row 2026-09-05 (168 website rows total). Jarvis intake cards were never affected. SIDD'S DECISIONS 2026-09-22: (1) NO Telegram alerts for forwarded mail (worker const MS_GRAPH_TELEGRAM_ALERTS = false, commit e335316), so the WEBHOOK_OUTBOX_* keys are NOT needed for ms-graph (Formspree and Quo still need them); (2) the poller NEVER marks mail read (his inbox). Rollout: worker deploy e335316 -> 657f45c0 (02:32 UTC, tick ok); Jarvis PR #178 squash-merged as 65dad9b (autodeploy 02:39 UTC, 1397 passed, poller 'configured: no'); droplet .env MS_GRAPH_CLIENT_STATE = 64 hex characters generated on the droplet and never printed (backup .env.bak-pre-clientstate-20260923T024010Z), outlook-poller restarted 02:40:10 'configured: yes'; the value piped droplet -> `npx wrangler secret put MS_GRAPH_CLIENT_STATE` -> version 17daf095 (02:40:33). Probe: a wrong clientState now answers 401 (was 503). No forward happened by 02:45 UTC (quiet night); a droplet watcher writes the first forward result to /tmp/webhook_first_forward.txt (12 hours). The poller also caps the forward body at 28,000 UTF-8 bytes (worker 413 limit 65,536), and tests/conftest.py clears MS_GRAPH_CLIENT_STATE so the deploy gate can never send a real forward. Vegas is now kept as a market on webhook lead rows. KILL: `wrangler secret delete MS_GRAPH_CLIENT_STATE` stops NEW intake only; rows already in webhook_intake_queue keep processing on the */5 cron (no attempt cap; the table is RPC-only, the service key gets 403). ROLLBACK TRAP: secrets survive code rollbacks. Rolling Claudia back BELOW 657f45c0 (ffd2a5de, 55ae3c69, 472e2778 and older) while MS_GRAPH_CLIENT_STATE is set turns the ms-graph Telegram alerts back on; they throw 503 on the missing outbox keys AFTER writing the row and retry for ever with a Haiku call each time. Delete MS_GRAPH_CLIENT_STATE FIRST. Not recovered: leads refused 2026-09-06 to 2026-09-23 never became dashboard rows (no backfill; a read-only list was offered). Formspree's direct route is still 503 (FORMSPREE_WEBHOOK_SIGNING_SECRET unset). Claudia's chat prompt VALID MARKETS still lacks vegas (worker.js about line 52).
